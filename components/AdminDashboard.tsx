...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ adminUser, onLogout, onLeadsSeen }) => {
  ...
  const closingPitch = useMemo(() => {
    ...
    if (emq < 6) return { title: "Priorité : Signal", text: `Son Pixel est aveugle (Score EMQ: ${emq}/10). Il dépense ${formatCurrency(budget)}/mois à l'aveugle. Vendez-lui SOS Signal.` };
    if ((ltv / pmv) < 1.3) return { title: "Priorité : LTV", text: `Sa LTV est trop faible. Proposez d'activer son backend pour doubler la valeur client.` };
    return { title: "Priorité : Scaling", text: "Tous les voyants sont au vert. Sécurisez sa montée en charge." };
  }, [selectedLead]);
  ...
  const getStatusDot = (type: 'signal' | 'ltv' | 'scaling', lead: LeadData) => {
    if (!lead.lastSimulation) return <div className="w-2 h-2 rounded-full bg-slate-100" />;
    ...
    if (type === 'signal') {
      ...
      color = ratio > 1.2 ? 'bg-emerald-500' : 'bg-red-500';
    } else if (type === 'scaling') {
      color = parseFloat(inputs.currentRoas) > parseFloat(inputs.targetRoas) ? 'bg-emerald-500' : 'bg-amber-500';
    }
          ...
          <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0 z-10">
              <div className="flex items-center gap-6">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setFilterType('all')} className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest border transition-all ${filterType === 'all' ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>Tous</button>
              ...
              <div className="relative w-80">
                ...
                <input type="text" placeholder="Rechercher par nom ou email..." className="w-full bg-slate-50 border border-slate-200 py-2 rounded-xl text-xs font-medium pl-10 outline-none focus:ring-2 ring-indigo-500/20 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
          </header>
          ...
          <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
                  {loading && leads.length === 0 ? (
                    <div className="text-center py-20 opacity-40 uppercase font-black text-[10px]">Chargement des données...</div>
                  ) : filteredLeads.length === 0 ? (
                    <div className="bg-white rounded-[2rem] p-20 text-center border-2 border-dashed border-slate-200">
                       <p className="text-4xl mb-4">🏜️</p>
                       <p className="font-black uppercase text-sm">Aucun prospect</p>
                    </div>
                  ) : (
                      ...
                      <table className="min-w-full divide-y divide-slate-100">
                          ...
                          <tbody className="divide-y divide-slate-100">
                              {filteredLeads.map((lead) => (
                                <tr key={lead.user.id} onClick={() => handleSelectLead(lead)} className={`cursor-pointer transition-all hover:bg-slate-50 ${selectedLead?.user.id === lead.user.id ? 'bg-indigo-50' : ''}`}>
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <div className="flex flex-col items-center gap-1">
                                           ...
                                           {(lead.user.consultingValue || 0) === 0 && (lead.user.purchasedProducts?.length || 0) === 0 && <span className="text-[7px] bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full font-black uppercase">Prospect</span>}
                                        </div>
                                    </td>
                                    <td className="px-4 py-4 text-center">
                        ...
                        <div className="p-8 space-y-8 animate-fade-in flex-1 overflow-y-auto">
                               ...
                               <div className="bg-indigo-50 p-6 rounded-[2rem] border border-indigo-100 flex flex-col shadow-sm">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                     {(selectedLead.user.purchasedProducts?.length || 0) > 0 ? (
                                       ...
                                       <p className="text-[9px] text-slate-400 italic font-medium">Aucun achat.</p>
                                     )}
                                  </div>
                                </div>
                            </div>
                            ...
                            {selectedLead.lastSimulation ? (
                                ...
                                <div className="bg-white border border-slate-100 p-8 rounded-[2rem] space-y-6 shadow-sm">
                                   <div className="flex justify-between items-center">
                                      <div className="max-w-[70%]">
                                         ...
                                         <p className="text-lg font-black text-slate-900 uppercase italic truncate">{selectedLead.lastSimulation.name}</p>
                                      </div>
                                   ...
                                   <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-[1.5rem] shadow-inner">
                              ...
                              <div className="bg-slate-50 rounded-[2rem] p-12 text-center border border-dashed border-slate-200">
                                ...
                                <p className="text-[10px] text-slate-400 italic max-w-[200px] mx-auto">Cet utilisateur n'a pas encore lancé de diagnostic.</p>
                              </div>
                            )}
                            ...
                            <div className="space-y-4 pb-12">
                               ...
                               />
                               <button onClick={handleSaveNote} className="w-full bg-slate-900 text-white py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-indigo-600 transition-all">Enregistrer Note &rarr;</button>
                            </div>
                        </div>
                      )}
                    </div>
