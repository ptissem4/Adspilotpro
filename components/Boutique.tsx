...
export const Boutique: React.FC<BoutiqueProps> = ({ onNotification, onBack }) => {
  ...
  const handleWaitlistSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Appel du service qui gère la BDD et la Notification Email (Admin)
      await AuthService.addToWaitlist(email);
      
      if (onNotification) onNotification("Inscription validée ! L'alerte a été envoyée au QG. 🚀", "success");
      setShowWaitlist(false);
      setEmail('');
    } catch (error) {
      console.error("Waitlist error:", error);
      if (onNotification) onNotification("Erreur lors de l'inscription.", "error");
    } finally {
  ...
  return (
    <div className="relative min-h-screen">
      {/* Navigation Header */}
      <nav className="fixed top-0 left-0 w-full h-20 z-50 px-6 md:px-12 flex items-center justify-between bg-slate-50/90 backdrop-blur-md border-b border-slate-200">
        <Logo onClick={onBack} className="cursor-pointer scale-75 origin-left" />
        <button 
          onClick={onBack} 
          className="group flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors px-4 py-2 rounded-xl hover:bg-white border border-transparent hover:border-slate-200"
        >
          <span className="text-[10px] font-black uppercase tracking-widest group-hover:-translate-x-1 transition-transform">← Retour</span>
        </button>
      </nav>

      <div className="max-w-6xl mx-auto px-6 pt-32 pb-40 animate-fade-in space-y-24">
        {/* HEADER */}
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in">
             <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 md:p-12 text-center space-y-8 relative shadow-2xl">
                ...
                <form onSubmit={handleWaitlistSubmit} className="space-y-4">
                   <input 
                     type="email" required 
                     className="w-full bg-slate-50 border border-slate-100 rounded-xl px-5 py-4 text-sm font-medium outline-none focus:ring-2 ring-indigo-500 disabled:opacity-50"
                     ...
                     autoFocus
                   />
                   <button type="submit" disabled={isSubmitting} className="w-full bg-slate-900 text-white py-5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-3">
                     {isSubmitting ? (
                        <>
